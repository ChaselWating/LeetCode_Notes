...
//    }
    ...
    public void addAtIndex(int index, int val) {
    	if(index < 0 || index > size) {		//是size而不是size-1，考虑index=size时直接插入到末尾
    		return;
    	} else {
    		ListNode p = new ListNode(val);
        	ListNode cur = this.dummyhead;		//dum 1 2 3 4
        	for(int i = 0; i < index; i++) {	//是index而不是index-1，因为dummynode不是0个
    			cur = cur.next;					//要将cur停在要插入的前一位	
    		}
        	p.next = cur.next;
        	cur.next = p;
        	size++;
